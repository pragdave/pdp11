...
import { MachineState, MSMemory, MSRegisters, RunningState } from "./machine_state"
...
export class Emulator implements EmulatorDecoders {
  ...
  constructor(private dispatcher: Dispatcher) {
    this.machineState = new MachineState(this.dispatcher)
    this.registers    = this.machineState.registers
    this.memory       = this.machineState.memory
  }
  ...
  step() {
    ...
    const deltas = this.machineState.capture_updates(() =>
      this.decodeAndRun(this.fetchAtPC())
    )
    switch (deltas.running_state) {
      case RunningState.Paused:
        console.error(`Ignoring unlikely paused state`)
        break
      case RunningState.Running:
        deltas.running_state = this.machineState.runningState = RunningState.Paused
        break
      default:
    ...
    for (let desc of decodeTable) {
      ...
      if (opcode || desc.mask === 0o177777) {
        ...
        if (handler) {
          ...
          this[decoder](handler, instruction)
          return
        }
      }
    }
